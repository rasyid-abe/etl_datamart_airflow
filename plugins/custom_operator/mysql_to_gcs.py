...
from custom_operator.sql_to_gcs import BaseSQLToGCSOperator
...
class MySQLToGCSOperator(BaseSQLToGCSOperator):
    ...
    def convert_type(self, value, schema_type: str, **kwargs):
        ...
        * Binary type fields are converted to integer if given schema_type is
        ...
        if value is None:
            return value
        if isinstance(value, datetime) and self.export_format != "parquet":
            value = str(value)
        elif isinstance(value, timedelta)  and self.export_format != "parquet":
            value = str((datetime.min + value).time())
        elif isinstance(value, Decimal):
            value = float(value)
        elif isinstance(value, date) and self.export_format != "parquet":
            if schema_type == "DATE":
